			...
			$(document).on('change', "input#qty", function(){
				...
				$.ajax({
					...
					success: function(response){
						if (response && typeof response === 'string') {
							$(".shipping-price span").html(response);
						} else if (response && response.data) {
							$(".shipping-price span").html(response.data);
						}
					},
					error: function(xhr, status, error) {
						console.error('Erreur AJAX (shipping):', error);
					}
				});
				...
				$.ajax({
					...
					success: function(result) {
						if (result && result.preparationDate) {
							$("#preparation-time").addClass('success').html(result.preparationDate);
						}
					},
					error: function(xhr, status, error) {
						console.error('Erreur AJAX (preparation time):', error);
					}
				});
			});
...
// Partie 2 - Suite du code après l'initialisation
...
require([
	...
	'mage/cookies',
	'mage/url'
], function($, tmp1, cookies, urlBuilder){
	if($(window).width() <= 768){
	...
	$(document).on('click', '#product-addtocart-button, #product-updatecart-button', function(){
	...
	$(document).on('click', '.open-product-designer', function(){
		...
		require([
			...
			// Stocker l'instance dans une variable accessible
			window.FancyProductDesigner = FancyProductDesigner2;
			...
			// Partie 3 - Suite du code - Options et initialisation du designer
...
var $yourDesigner = $('#clothing-designer'),
	pluginOpts = {
		...
		actions:  {
			...
			'right': [],
			'bottom': [],
			'left': ['undo','redo','manage-layers', 'zoom', 'reset-product']
		},
		langJSON: $("input#baseurl").val() + 'media/productdesigner/default.json',
		templatesDirectory: $("input#baseurl").val() + 'media/productdesigner/templates/',
...
// Partie 4 - Suite du code - Gestion des événements du designer
...
$yourDesigner.on('ready', function(){
	...
	$yourDesigner.on('elementAdd', function(e, el){
		...
		if(!data) {
			console.error('Données JSON introuvables pour le libellé:', label);
			return;
		}
		
		if(el.text != null){
			yourDesigner.setElementParameters({
				top: parseFloat(data.output_y1) + ((parseFloat(data.output_y2) - parseFloat(data.output_y1)) / 2),
				left: parseFloat(data.output_x1) + ((parseFloat(data.output_x2) - parseFloat(data.output_x1)) / 2),
			}, el);
			if($("input#fixed_printing_color").val() != ''){
				yourDesigner.setElementParameters({
					color: $("input#fixed_printing_color").val(),
...
// Partie 5 - Suite de la gestion des événements
...
if(el.source != null && (el.source.indexOf('http') > -1 || el.source.indexOf('data:image') > -1)){
	...
	if(!jsonData) {
		console.error('Données JSON introuvables pour le libellé (image):', label);
		return;
	}
...
// Partie 6 - Suite des événements du designer
...
$yourDesigner.on('layersListUpdate', function() {
	...
	$('.fpd-custom-layers-container [data-module="manage-layers"]').each(function(i, moduleWrapper) {
...
// Partie 7 - Suite des événements et interactions
...
$yourDesigner.on('elementSelect', function() {
	...
	var elementContainer = $(".fpd-list-row[id='"+yourDesigner.currentElement.id+"']");
	...
	var row = $(".fpd-list-row[id='"+yourDesigner.currentElement.id+"']");
...
// Partie 8 - Chargement des templates et design
...
setTimeout(function(){
	...
	$.each(views, function(index, view){
		...
		if(!data) {
			console.error('Données JSON introuvables pour le libellé (surcharge):', label);
			return;
		}
...
// Partie 9 - Sélection de vue et boutons
...
$(document).on('click', '.fpd-curved-text-switcher', function(){
	...
	console.log(yourDesigner.currentElement);
	setTimeout(function(){
		$('#clothing-designer').trigger('layersListUpdate');
	}, 1500);
	setTimeout(function(){
		$('#clothing-designer').trigger('layersListUpdate');
		setTimeout(function(){
			$(".fpd-list-row[id='"+elementId+"']").trigger('click');
			$(".fpd-tool-curved-text").trigger('click');
		}, 1000);
	}, 3000);
});
// Partie 10 - Alignement des éléments
...
$(document).on('click', '.fpd-align-left', function(){
	...
	if(!data) {
		console.error('Données JSON introuvables pour le libellé (align):', label);
		return;
	}
...
$(document).on('click', '.fpd-align-bottom', function(){
	...
	yourDesigner.setElementParameters({
		top: parseFloat(data.output_y2) - (el.getBoundingRect().height / 2)
	}, el);
});
...
// Partie 11 - Bouton checkout et processus d'ajout au panier

// Checkout button avec getProduct()
$('#checkout-button').click(function(){
	// Vérifier si des éléments de design sont présents
	var views = yourDesigner.viewInstances;
	var hasDesign = false;
	$.each(views, function(index, view){
	...
	// Afficher le progrès d'ajout au panier
	...
	yourDesigner.getViewsDataURL(function(images) {
		$.each(product, function(index, productData){
			...
			var elements = []; // Simplifié pour éviter les erreurs
			...
			cart[count] = { 
				...
				'width': productData.options.stageWidth, 
				'height': productData.options.stageHeight, 
				'x': x, 
				'y': y, 
				'outputx1': outputX1, 
				'outputx2': outputX2, 
				...
				$.ajax({
					...
					success: function(checkresult) {
						if(typeof(checkresult.count) != 'undefined' && checkresult.count > 0){
							// Ajouter au panier une fois sauvegardé
							...
							$.ajax({
								...
								data: { 
									'cart': cart, 
									'qty': parseInt($("input.fpd-qty").val()), 
									'connect_id': result, 
									'isupdatequoteitem': $('input#isUpdateQuoteItem').val(), 
									'number_chains': numberChains, 
									'surchargeAmount': $("input#surchargeAmount").val(), 
									'options': optiondata,
									'form_key': $.mage.cookies.get('form_key')
								},
								dataType: 'json',
								success: function(result) {
									// Redirection au panier après ajout réussi
							...
							alert('Votre design n\'a pas pu être sauvegardé, veuillez réessayer.');
							$("div.pd-cart-progress-container").hide();
						}
					},
					error: function(xhr, status, error) {
						console.error('Erreur AJAX (vérification id):', error);
						alert('Erreur lors de la vérification de l\'ID de connexion. Veuillez réessayer.');
						$("div.pd-cart-progress-container").hide();
					}
				});
			},
			error: function(xhr, status, error) {
				console.error('Erreur AJAX (sauvegarde):', error);
...
// Partie 12 - Gestion de la réinitialisation et des prix
...
$(document).on('change', 'input.fpd-qty', function(){
	...
	$.ajax({
		...
		error: function(xhr, status, error) {
			console.error('Erreur AJAX (date de préparation):', error);
		}
	});
});
$(".fpd-qty").trigger('change');

// Événement lorsque le prix change
$yourDesigner.on('priceChange', function(evt, price, currentPrice) {
	...
	$.each(views, function(index, view){
		...
		if(!data) {
			console.error('Données JSON introuvables pour le libellé (prix):', label);
			return;
		}
...
// Partie 14 - Fonctions utilitaires et fin du code

// Fermeture des blocs require
});
...
// Optimiser la vérification du taux d'expédition
setInterval(function(){
	if(getShippingRateVar !== false){
		...
		$.ajax({
			...
			error: function(xhr, status, error) {
				console.error('Erreur AJAX (shipping rate):', error);
			}
		});
	}
}, 1000);
// Partie 15 - Événements pour le designer et finalisation

// Déclencher automatiquement l'ouverture du designer si nécessaire
...
// Gestion des événements pour les champs de texte et pourcentages
...
$(document).on('change', ".image-scale-percentage input", function(){
	...
	var el = yourDesigner.currentElement;
	if (!el) {
		console.error('Aucun élément sélectionné pour le changement d\'échelle');
		return;
	}
	
	if(el.origScale > 0){
		var scaleFactor = percentage / 100;
